...
class GlobalData:
    ...
    # ### Final API
    def get_common_topics(self, user_topics, article_topics):
        return list(set(user_topics) & set(article_topics))
        
    def get_n_articles_with_topic(self, topic, n=10, all=False):
        if n <= 0:
            n = 10
        df = self.cnt.loc[self.cnt.apply(lambda x: topic in x['topics'], axis=1)]
        if all:
            return df
        return df.sample(n)

    def get_10_articles_for_new_user(self, user_id, topics=[]):
        import pandas as pd
        keep = ['item_id_adj', 'title', 'topics', 'text_description']
        if len(topics) == 0:
            return self.cnt.sample(10)[keep]
        ...
        for topic in topics:
            df = pd.concat([df, self.get_n_articles_with_topic(topic, n=10)])
    ...
    def get_top_10_articles_for_user(self, user_id):
        if not self.consumer_id_helper.is_known_id(user_id):
            return self.get_10_articles_for_new_user(user_id)
        return self.als.get_articles_for_user_from_als(user_id, n=10)
    ...
    def get_more_articles_for_user(self, article_id, user_id):
        ...
        return self.hybrid.get_articles_matching_article_from_als_content_hybrid(article_id, n=10, ignore=to_filter)

    def get_article_by_id(self, article_id):
        try:
            return self.cnt[self.cnt['item_id_adj'] == article_id].iloc[0][['item_id_adj', 'title', 'topics', 'text_description']].to_dict()
        except:
            return None
...
class ALS:
    ...
    def train_user(self):
        ...
        self.best_als = AlternatingLeastSquares(
            factors=best_user_based_f, 
            regularization=best_user_based_r, 
            iterations=best_user_based_i, 
            calculate_training_loss=False
        )
        self.best_als.fit(train)

    def get_articles_for_user_from_als(self, user_id, n=20):
        import pandas as pd
        id, scores = self.best_als.recommend(user_id, self.sparse_user_article[user_id], N=50, filter_already_liked_items=True)
    ...
    def get_articles_matching_article_from_als(self, article_id, n=20, all=False):
        import pandas as pd
        ids, scores = self.best_als.similar_items(
...
def load(data, data_dir):
    ...
    # sites\onenews\onenews\static\data\consumer_transanctions.csv

    data.txns = pd.read_csv(os.path.join(data_dir, 'consumer_transanctions.csv'))
    data.cnt = pd.read_csv(os.path.join(data_dir, 'platform_content.csv'))
